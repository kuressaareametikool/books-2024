<x-layout>
    <x-slot:title>Edit Author</x-slot>
    <form action="{{ route('authors.update', $author) }}" method="POST">
        ...
        <div class="max-w-sm p-2 flex flex-col gap-2">
            <div>
                ...
                <input value="{{$author->first_name}}" id="first_name" placeholder="John" name="first_name"
            ...
            <div>
                ...
                <input value="{{$author->last_name}}"  type="text" id="last_name" placeholder="Doe" name="last_name"
                    class="mt-1 w-full rounded-md border-gray-200 shadow-sm sm:text-sm" />
                @error('last_name')
                    <span class="text-xs text-red-500">{{ $message }}</span>
                @enderror
