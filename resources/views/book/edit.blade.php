    ...
    <form action="{{ route('books.update', $book) }}" method="POST">
        ...
        <div class="max-w-sm p-2 grid grid-cols-3 gap-2">
            <div class="col-span-3">
                ...
                <input type="text" id="title" placeholder="Test" name="title" value="{{ $book->title }}"
            ...
            <div>
                ...
                <input type="text" id="language" placeholder="ET" name="language" value="{{ $book->language }}"
                    class="mt-1 w-full rounded-md border-gray-200 shadow-sm sm:text-sm" />
                @error('language')
                    <span class="text-xs text-red-500">{{ $message }}</span>
                @enderror
            ...
            <div class="">
                ...
                <select class="mt-1 w-full rounded-md border-gray-200 shadow-sm sm:text-sm" name="type"
                    id="type">
                    @foreach ($types as $key => $value)
                        <option value="{{ $key }}" @selected($book->type === $key)>
                            {{ $value }}</option>
                    @endforeach
                </select>
                @error('type')
            ...
            <div>
                ...
                <input type="text" id="price" placeholder="1234" name="price" value="{{ $book->price }}"
                    class="mt-1 w-full rounded-md border-gray-200 shadow-sm sm:text-sm" />
                @error('price')
                    <span class="text-xs text-red-500">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="text" id="pages" placeholder="1234" name="pages" value="{{ $book->pages }}"
                    class="mt-1 w-full rounded-md border-gray-200 shadow-sm sm:text-sm" />
                @error('pages')
                    <span class="text-xs text-red-500">{{ $message }}</span>
                @enderror
            ...
            <div class="col-span-3">
                ...
                <textarea type="text" id="summary" placeholder="1234" name="summary" rows="5"
                    class="mt-1 w-full rounded-md border-gray-200 shadow-sm sm:text-sm">
